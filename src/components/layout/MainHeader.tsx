        ...
        <header className="flex-none border-b border-gray-100 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70">
            <div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between gap-6 px-4 sm:px-6 lg:px-8">
                    ...
                    {NAV_ITEMS.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.label}
                                href={item.to ?? "/"}
                                className={`transition-colors hover:text-gray-900 ${isActive ? "text-gray-900 font-semibold" : ""}`}
                            >
                                {item.label}
                            </Link>
                        );
                ...
                <div className="hidden w-[140px] justify-end lg:flex">
                    <Link
                        href="/login"
                        className="flex items-center gap-2 rounded-full border border-green-500 bg-white px-5 py-2 text-green-600 transition-colors hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-500/40"
                        aria-label="로그인"
                    >
            ...
            {isMenuOpen && (
                <div className="border-t border-gray-100 bg-white lg:hidden">
                    <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-4 pb-6 pt-4 sm:px-6">
                        {NAV_ITEMS.map((item) => {
                            ...
                            return (
                                <Link
                                    key={`mobile-${item.label}`}
                                    href={item.to ?? "/"}
                                    className={`text-sm font-medium transition-colors ${isActive ? "text-gray-900" : "text-gray-700"}`}
                                    onClick={() => setIsMenuOpen(false)}
                                >
                                    {item.label}
                                </Link>
                        ...
                        <Link
                            href="/login"
                            className="flex items-center justify-center gap-2 rounded-full border border-green-500 bg-white px-5 py-2 text-green-600 transition-colors hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-500/40"
                            aria-label="로그인"
                            onClick={() => setIsMenuOpen(false)}
                        >
                            <img src="/user-icon.svg" alt="사용자" className="h-5 w-5" />
